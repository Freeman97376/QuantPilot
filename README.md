...
北京旅游 Agent 是面向本地出行/游玩的智能路线规划工作台。用户用自然语言输入游玩目标，系统结合北京 POI、餐饮/文化混排、UGC 评论特征和约束验证，生成可直接执行、可解释、可动态调整的路线方案。

本项目默认使用本地静态数据，不接入实时排队、实时地图导航或实时营业状态；距离、交通时间和排队风险均应作为估算或历史评论信号展示。

## 内置北京数据集

仓库已内置最小可运行北京 POI/UGC 数据集，路径为 `travel-data/processed`。默认情况下不需要额外下载 `repo` 目录即可启动路线规划接口。
...
```env
TRAVELPILOT_DATA_ROOT="/absolute/path/to/processed"
```
