...
- `action: "prepare-analysis"`：重新确认标的是当前日线命中候选后，按需准备 1 分钟/5 分钟分析数据。

首次进入智能策略页时，页面预填一个可执行示例，并将“生成草案 -> 执行筛选”合并为一个主动作。未配置 DeepSeek 或远端请求失败时，服务端会在草案中标注原因并降级为本地确定性解析；如果出现冲突条件、不支持词或只有样本/ST/涨停等默认风控项，前端必须停在复核阶段，不得自动运行。
